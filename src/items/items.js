...
function ListItems() {
  ...
  const deleteItems = (id) => {
    Axios.delete(`http://localhost:3001/items/deleteItem/${id}`).then(
      (response) => {
        setItemsList(
  ...
  useEffect(() => {
    if (localStorage.getItem("user")) getItems();
  }, []);
      ...
      <tbody>
        {ItemsList.map((val) => (
          <tr styl={{ marginLeft: "40%" }}>
            ...
            <td style={{ backgroundColor: "white" }}> {val.description}</td>
        ...
        <Link to={`/items/add/`}>
          {localStorage.getItem("user") ? (
            <Button style={{ marginLeft: "420%" }}>Add Author</Button>
          ) : null}
        </Link>
      </tbody>
    </table>
